...
async function main(): Promise<void> {
  ...
  await check("first login: admin API is blocked until the password is changed", async () => {
    ...
    const blocked = await admin.get("/leads");
    expect(blocked.status === 403, `expected 403, got ${blocked.status}`);
    expect(
      blocked.body.error.code === "PASSWORD_CHANGE_REQUIRED",
      `code was ${blocked.body.error.code}`
    );
    return "blocked with PASSWORD_CHANGE_REQUIRED";
  });
